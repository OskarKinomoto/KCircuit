...
TabWidget::TabWidget()
{
  ...
  connect(this->tabBar(), SIGNAL(tabMoved(int,int)), this, SLOT(tabMoved(int,int)));
...
bool TabWidget::save()
{
  if(circuit()->hasPath()) circuit()->save();
  else
    {
      QString fileName = QFileDialog::getSaveFileName(this->parentWidget(), tr("Save as..."), K::home + "/KCircuit/SAMPLE/", tr("KCircuit File (*.ktc)"), 0, QFileDialog::DontUseNativeDialog);
  ...
  if(static_cast<MainWidget *>(this->widget(i))->circuit->isModyfied())
    {
      ...
      if(!(*saveAll))
        {
          QMessageBox msgBox;
          msgBox.setText(tr("<center><h3>The document has been modified.</h3></center>"));
          msgBox.setInformativeText(tr("<center>Do you want to save your changes?</center>"));
...
void TabWidget::tabMoved(int, int)
{
  this->updateIndex();
}
