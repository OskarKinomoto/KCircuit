  ...
  if(label != "")
    {
      ...
      if(gsRendered != K::gs)
       ...
       switch(angle)
         {
         case 0:
           p.drawImage(-labelImg->width()/2,-labelImg->height()/2 + 2*K::gs,*labelImg);
           break;
         case 180:
           p.drawImage(-labelImg->width()/2,-labelImg->height()/2 - 2*K::gs,*labelImg);
           break;
         case 90:
           p.drawImage(-labelImg->width() - K::gs,-labelImg->height()/2,*labelImg);
           break;
         case 270:
           p.drawImage(K::gs,-labelImg->height()/2,*labelImg);
           break;
         case 45:
           p.drawImage(-labelImg->width() - K::gs,labelImg->height()/2 - K::gs,*labelImg);
           break;
         case 135:
           p.drawImage(-labelImg->width() - K::gs,-labelImg->height()/2 - K::gs,*labelImg);
           break;
...
ObjectResistor::TYPE ObjectResistor::getTyp() const
{
  return typ;
}

void ObjectResistor::setTyp(int value)
{
  typ = ObjectResistor::TYPE(value);
}


QString ObjectResistor::getLabel() const
{
  return label;
}
