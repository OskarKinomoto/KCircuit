...
ObjectCapacitor::ObjectCapacitor(QPoint p, quint16 a, quint32 id)
    : ObjectBase(p, a, K::CAPACITOR, id) {
  name = "Capacitor " + QString::number(id);
}

ObjectCapacitor::ObjectCapacitor(QDataStream &in)
    : ObjectBase(in, K::CAPACITOR) {
  loadSettings(in);
}
...
ObjectSettingsAbstract *ObjectCapacitor::settings(Circuit *) { return nullptr; }

QString ObjectCapacitor::getLabel() const { return label; }
...
ObjectCapacitor::TYPE ObjectCapacitor::getTyp() const { return typ; }

void ObjectCapacitor::setTyp(int value) { typ = ObjectCapacitor::TYPE(value); }
