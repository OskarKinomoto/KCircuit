...
void ObjectWire::draw(QPainter &p)
{
  ...
  switch(mode)
    ...
    case K::WIRE_TAXI:
      int i = 1;
      for(auto itr = points.begin(), itr2 = itr + 1; itr2 < points.end(); ++itr, ++itr2, ++i)
        if(itr->x() == itr2->x() || itr->y() == itr2->y())
        ...
        else
          {
            ...
            p.drawLine(itr->x()*K::gs, itr->y()*K::gs, midX*K::gs, midY*K::gs),
                p.drawLine(midX*K::gs, midY*K::gs, itr2->x()*K::gs, itr2->y()*K::gs);
          }
...
{
  QPoint tmp(p.x()/ K::gs + .5, p.y() / K::gs + .5);
  if(points.back() == tmp) return false;
  points.back() = tmp;
  return true;
}
    ...
    case K::WIRE_TAXI:
      for(size_t i = 0, j = 1; j < points.size(); ++i, ++j)
        if(points.at(i).x() == points.at(j).x() || points.at(i).y() == points.at(j).y())
          ...
          {
            QPoint mid(points.at(typ.at(j) ? i : j).x(), points.at(typ.at(j) ? j : i).y());
...
ObjectSettingsAbstract *ObjectWire::settings(Circuit *)
{
  return nullptr;
}

bool ObjectWire::isPointOverWirePart(QPoint &a, QPoint &b, QPointF &c, float dist)
{
  int px = b.x() - a.x();
  int py = b.y() - a.y();
